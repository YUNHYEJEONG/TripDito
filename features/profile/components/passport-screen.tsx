...
const pageStyle: CSSProperties = {
  ...
  backgroundImage:
    "radial-gradient(circle at 16% 14%, rgba(65, 126, 144, 0.1) 0 0.7px, transparent 1px), radial-gradient(circle at 76% 62%, rgba(205, 151, 112, 0.07) 0 0.7px, transparent 1px), linear-gradient(145deg, var(--passport-paper-hi) 0%, var(--passport-paper-mint) 48%, var(--passport-paper-warm) 100%)",
...
export function PassportScreen({
  ...
  const archivedTrips = getCompletedPassportTrips(trips);
  const passportLoading = !hydrated || isLoading;
    ...
    >
      {embedded ? (
        ...
        <header className="flex min-h-16 items-end justify-between gap-3 px-2 pb-1">
          ...
          <p className="pb-1 text-right text-[12px] font-semibold text-ink-2">
            <span role="status">
              {passportLoading
                ? "기록 불러오는 중"
            ...
            {!passportLoading && archivedTrips.length > 2 ? (
              <span
                aria-hidden
                className="mt-0.5 block text-[11px] font-medium text-ink-3"
              >
                밀어서 넘기기
              </span>
            ) : null}
